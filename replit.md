...
- **Cloud BYO API key (Google Gemini only)**: works identically — `cloudProviders.ts` makes DIRECT client `fetch` calls to the Gemini `generativelanguage.googleapis.com/v1beta` API (streaming via `streamGenerateContent?alt=sse`), which works inside Tauri's webview. This is intentionally NOT routed through the Replit AI proxy — it's a user-supplied free-tier key. Vision-capable Gemini models (e.g. `gemini-2.5-flash`, `gemini-1.5-pro`) accept image attachments — `CloudMessage.content` is a parts array (`inlineData` base64 for images), and `cloudModelSupportsVision()` gates the in-chat image picker. OpenAI and Anthropic were removed; `CloudProvider` is the single literal `"gemini"` and the config holds only `geminiKey`/`geminiModel`.
...
- **Local-first, cloud-optional**: both builds run open-source models on the user's own hardware out of the box, with no backend, account, or telemetry. As of the BYO-API-key feature, users *can* paste their own free Google Gemini API key (from aistudio.google.com/apikey — no credit card needed for the free tier) in Settings / Connections and route specific messages through Gemini — strictly opt-in, per-message, with a clear "Sending to Google (Gemini)" badge. The key lives in localStorage.
