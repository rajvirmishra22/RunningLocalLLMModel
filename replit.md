...
- **Local vision (desktop only)**: vision-flagged catalog entries (Llama 3.2 11B Vision, MiniCPM-V 2.6, Qwen2-VL 7B, LLaVA 1.6, Moondream 2, Gemma 3 4B) download TWO files — the LLM GGUF and a companion `mmproj` (CLIP-style image projector). At chat time, when the active model has `vision: true` and the message includes images, the desktop `webllmService.streamChat` routes to the new `chat_with_images` Tauri command, which spawns the `llama-mtmd-cli` sidecar (from `src-tauri/binaries/llama-mtmd-cli-<target>`) with `--mmproj`, `-m`, and one `--image <tempfile>` per attachment, streaming stdout back as `token` events. The sidecar binary is NOT bundled by the workspace build — it's a precompiled llama.cpp tool the desktop installer ships via Tauri's `externalBin` (see `desktop/README.md` for where to drop it per target triple). FFI was rejected because `llama-cpp-2` doesn't expose llava/mtmd bindings and hand-rolling them against the C API is brittle across llama.cpp versions; the sidecar gives us mtmd support today with a stable contract.
